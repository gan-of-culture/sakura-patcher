...
ALL_PATCHES = [
    ...
    },
    {
        ...
        "hashes": {"assets.rpa": "977863274203BDEABA6C3764A4CF0E49"},
    },
    {
        "game": "Sakura Cupid",
...
class Patcher(QMainWindow):
    def __init__(self):
        ...
        self.pathToGames.setObjectName("pathToGames")
    ...
    def retranslateUi(self):
        self.setWindowTitle(QCoreApplication.translate("Patcher", "Patcher", None))
        self.patch.setText(
            QCoreApplication.translate("Patcher", "Patch selected", None)
        )
        self.selectAll.setText(
            QCoreApplication.translate("Patcher", "Select all", None)
        )
        self.deselectAll.setText(
            QCoreApplication.translate("Patcher", "Deselect all", None)
        )
        self.refresh.setText(QCoreApplication.translate("Patcher", "Refresh", None))
        self.pathToGames.setText("")
        self.pathToGames.setPlaceholderText(
            QCoreApplication.translate("Patcher", "Path to your sakura games", None)
        )
        self.browse.setText(QCoreApplication.translate("Patcher", "...", None))

    # retranslateUi

    def browseFiles(self):
        dirName = QFileDialog.getExistingDirectory(
            self, "Open directory", os.path.abspath("~//Desktop//")
        )
    ...
    def updateItems(self):
        ...
        for game in allGames:
            if game.startswith("Sakura") or game.startswith(
    ...
    def runPatch(self):
        ...
        for idx, listobj in enumerate(self.listWidget.selectedItems()):
            patch = next(
                (item for item in ALL_PATCHES if item["game"] == listobj.text()), None
            )
                ...
                for root, directory, files in os.walk(cwd):
                    for file in files:
                        if any(file in k for k in patch["hashes"].keys()):
                            try:
                                os.rename(
                                    join(root, file), join(cwd, patch["game"], file)
                                )
